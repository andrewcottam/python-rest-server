...
import web, json, sys, re, datetime, urllib, psycopg2, ast, uuid, cPickle, logging, glob, serviceRequest, urllib 
...
GEE_IMAGE_SERVER_URL = "https://geeimageserver.appspot.com/getValuesForPoints?"
...
class createValidationSites:  # https://db-server-blishten.c9users.io/gee/createValidationSites?username=asd&sceneid=LANDSAT%2FLC8_L1T_TOA%2FLC81970502017085LGN00&scenedate=2017-03-26%2010%3A37%3A38.3802370Z&draftSites=%5B%5B-3.6105494473017585%2C14.629853605143476%2C%22Waterbodies%22%5D%5D&callback=dojo_request_script_callbacks.dojo_request_script8
    def GET(self):
        try:
            ...
            url = GEE_IMAGE_SERVER_URL + urllib.urlencode({'sceneid': sceneid, 'points': pointsArray})
            data = eval(urllib.urlopen(url).read())  # send these to GEE to get the pixel values
            if len(data) == 1:
